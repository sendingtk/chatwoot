...
describe AutoAssignment::InboxRoundRobinService do
  ...
  describe '#available_agent with inbox teams' do
    let!(:team) { create(:team, account: account) }
    let!(:agents) { create_list(:user, 5, account: account, role: :agent) }
    let!(:team_members) do
      agents.map do |agent|
        create(:team_member, user: agent, team: team)
      end
    end
    let!(:inbox_team) { create(:inbox_team, inbox: inbox, team: team) } # rubocop:disable RSpec/LetSetup]
    ...
    it 'gets the first available agent in allowed_agent_ids and move agent to end of the list' do
      expected_queue = [team_members[0].user_id, team_members[4].user_id, team_members[3].user_id, team_members[2].user_id,
                        team_members[1].user_id].map(&:to_s)
      described_class.new(inbox: inbox).available_agent(allowed_agent_ids: [team_members[0].user_id, team_members[4].user_id].map(&:to_s))
      expect(inbox_round_robin_service.send(:queue)).to eq(expected_queue)
    end
    ...
    it 'constructs round_robin_queue if queue is not present' do
      ...
      inbox_round_robin_service.available_agent
      # the service constructed the redis queue before performing
      expect(inbox_round_robin_service.send(:queue).map(&:to_i)).to match_array(team_members.map(&:user_id))
    end

    it 'validates the queue and correct it before performing round robin' do
      # adding some invalid ids to queue
      inbox_round_robin_service.add_agent_to_queue([2, 3, 5, 9])
      expect(inbox_round_robin_service.send(:queue).map(&:to_i)).not_to match_array(team_members.map(&:user_id))
      inbox_round_robin_service.available_agent
      # the service have refreshed the redis queue before performing
      expect(inbox_round_robin_service.send(:queue).map(&:to_i)).to match_array(team_members.map(&:user_id))
    end
    ...
    context 'when allowed_agent_ids is passed' do
      it 'will get the first allowed member and move it to the end of the queue' do
        expected_queue = [team_members[3].user_id, team_members[2].user_id, team_members[4].user_id, team_members[1].user_id,
                          team_members[0].user_id].map(&:to_s)
        expect(described_class.new(inbox: inbox).available_agent(
                 allowed_agent_ids: [
                   team_members[3].user_id,
                   team_members[2].user_id
                 ].map(&:to_s)
               )).to eq team_members[2].user
        expect(described_class.new(inbox: inbox).available_agent(
                 allowed_agent_ids: [
                   team_members[3].user_id,
                   team_members[2].user_id
                 ].map(&:to_s)
               )).to eq team_members[3].user
        expect(inbox_round_robin_service.send(:queue)).to eq(expected_queue)
      end
    end
  end

  describe '#remove_agents_from_round_robin from inbox team' do
    let!(:team) { create(:team, account: account) }
    let!(:agents) { create_list(:user, 5, account: account, role: :agent) }
    let!(:team_members) do # rubocop:disable RSpec/LetSetup
      agents.map do |agent|
        create(:team_member, user: agent, team: team)
      end
    end
    let!(:inbox_team) { create(:inbox_team, inbox: inbox, team: team) }

    it 'returns an empty array when inbox team is destroyed' do
      inbox_team.destroy
      expect(described_class.new(inbox: inbox).send(:queue)).to be_empty
    end
  end
end
